...
from app.core.exceptions import InvalidCredentials, UniqueConstraintViolation
...
@pytest.mark.asyncio
async def test_save_register_protocol(mock_uuid, mock_redis, mock_email_client):
    fake_user = UserRegister(
        email="randomuser@gmail.com", name="Random User", password="RandomPassword"
    )
...
async def test_search_for_user_return_true():
    ...
    async with mock_database as conn:
        with patch("app.routes.user_routes.db_connection", return_value=conn):
            ...
            assert (
                await user_routes.search_for_user("non_existing_email@gmail.com")
                is True
            )
...
def test_handle_register_req():
    fake_user = schemas.UserRegister(
        email="test_mail@gmail.com", password="test_password", name="Test User"
    )

    fake_db = MockDatabase()

    with patch("app.routes.user_routes.db_connection", return_value=fake_db):
        response = fastapi_test_client.post(
            "/api/user/register", json=fake_user.model_dump()
        )
